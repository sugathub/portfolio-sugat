...
function ExperienceItem({ exp, idx, start, end, scrollYProgress, layout }) {
  ...
  if (layout === "desktop") {
    return (
      <div className="relative flex flex-1 justify-center items-center min-w-0">
        ...
        {/* Connector line */}
        <motion.div
          className={`absolute ${idx % 2 === 0 ? "-top-8" : "-bottom-8"} w-[3px] bg-yellow-400/40`}
  ...
  return (
    <div className="relative flex items-start">
      ...
      <motion.article
        className="bg-gray-900 backdrop-blur border border-gray-700/70 rounded-xl p-5 w-[90vw] max-w-sm ml-6 shadow-lg"
        style={{ opacity, x }}
        transition={{ duration: 0.4, delay: idx * 0.15 }}
      >
...
const Experience = () => {
  ...
  const thresholds = useMemo(() => {
    return experiences.map((_, i) => (i + 1) / experiences.length)
  }, [experiences.length])
    ...
    <section id="experience" className="relative bg-black text-white">
      ...
      >
        ...
        <div className="sticky top-0 h-screen flex flex-col">
          ...
          <div className="flex flex-1 items-center px-6 pb-10">
